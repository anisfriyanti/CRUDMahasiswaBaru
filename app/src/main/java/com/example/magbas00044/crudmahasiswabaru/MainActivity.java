...
public class MainActivity extends AppCompatActivity {
    ...
    @BindView(R.id.recycle_mahasiswa)
    RecyclerView recycleMahasiswa;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ConfigRetrofit.service.lihat().enqueue(new Callback<ResponseGetMahasiswa>() {
            ...
            public void onResponse(Call<ResponseGetMahasiswa> call, Response<ResponseGetMahasiswa> response) {
                ...
                if (status == 1) {
                    List<DatanyaItem> dataMhs = response.body().getDatanya();
                    MahasiswaAdapter adapter = new MahasiswaAdapter(dataMhs);
                    recycleMahasiswa.setAdapter(adapter);
                    recycleMahasiswa.setLayoutManager(new LinearLayoutManager(MainActivity.this));
                }
            }
    ...
    private void dialog_input() {
        ...
        ednim= (EditText)alertLayout.findViewById(R.id.edNIM);
        ednama=(EditText)alertLayout.findViewById(R.id.edNama);
        edjurusan=(EditText)alertLayout.findViewById(R.id.edJurusan);

        AlertDialog.Builder alert= new AlertDialog.Builder(this);
        alert.setView(alertLayout);
        alert.setCancelable(false);
        alert.setTitle("Input data Baru");

        alert.setPositiveButton("Simpan", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(final DialogInterface dialog, int which) {
                String inputNim= ednim.getText().toString();
                String inputNama=ednama.getText().toString();
                String inputJurusan= edjurusan.getText().toString();


                ConfigRetrofit.service.tambah(inputNim, inputNama, inputJurusan).enqueue(new Callback<ResponseInsert>() {
                    @Override
